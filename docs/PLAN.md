...
- [x] 5.1 **SSO client integration** (2026-07-21): copied the `nexo-sso-client` template (unmodified per its hard rule), adapted only the allowed points (`intended` → `route('panel')`; `newUser` matches the users table as-is; `login` route exists). `firebase/php-jwt`, `config/nexo-sso.php`, `routes/nexo-sso.php` (required in `routes/web.php`, panel host), `add_nexo_id_sub_to_users_table` migration, `NexoSso*` services + controller, 16 AC-tested Pest tests (CFG/FLOW/LINK/SESS/DEGRADE) all green. `NEXO_SSO_ENABLED=false` default → standalone auth intact, no SSO routes. "Continue with Nexo ID" button on login (i18n en/es/pt). 106 tests green.
  ...
  - ✅ **Template iterable-type docblocks fixed upstream** (alvaro `3c86077`); resolver re-synced from the template and the temporary `phpstan.neon` ignore removed. Pint docblock spacing also fixed upstream (alvaro `09e7c7f`) → resolver re-synced **byte-for-byte canonical** and Pint-clean.
  - ✅ **Validated with nexo-id (2026-07-22):** redirect URI confirmed (`nexoshort.alvarocdev.com/auth/nexo/callback`, old nxo.li one removed); `sub`/`email`/`email_verified`/`name` all in the **id_token** (no userinfo call needed — matches the template); login page renders the `nexo_sso` error bag (Gate 3 dependency) — locked by a test. Nothing else blocking on the nexo-id side; back-channel logout is Phase-5-by-design (current local-logout semantics are correct). **nexo-id T4 done.** Remaining before the real flow: Phase 4 deploy + flipping SSO on at launch (+ optional local e2e against nexo-id `:8100`, coordinated via Alvaro).
...
- [x] 5.4a `audit-open-source` **pass (2026-07-22)** — history + HEAD clean: no secrets/keys/`.env`/sqlite ever tracked, no real IPs or internal infra (only `CHANGE_ME` placeholders); public domains + OIDC client_id are intentional. **One low finding:** commit author email `alvaro@mc4pc.com` would be public (owner's call; a GitHub noreply for future commits avoids it — no history rewrite needed).
