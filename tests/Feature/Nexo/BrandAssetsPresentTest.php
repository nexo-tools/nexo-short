...
// run scripts/generate-brand-assets.mjs after a mark change) keeps the suite
// green and only shows up as a broken favicon / blank OG card in production.
...
it('wires the favicon and the OG image into the panel chrome', function () {
    ...
    // Panel host only: the short host is asset-light by design (ADR-001).
    ...
    expect(str_contains($html, '/og-image.png'))->toBeTrue('og:image does not point at /og-image.png.');
});
