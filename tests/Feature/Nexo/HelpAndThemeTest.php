...
/**
 * The help center answers on the PANEL host: the short host isolates itself
 * from the panel chrome (ADR-001) and its {slug} catch-all 404s /help, which
 * HostChromeIsolationTest guards. This is the one adaptation the guardian
 * allows, and the path is still the canonical /help.
 */
function helpUrl(string $query = ''): string
{
    return panelUrl(HELP_URL.$query);
}

/** The public shell whose chrome must link the help center — panel host too. */
