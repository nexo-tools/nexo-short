...
Open source URL shortener of the Nexo ecosystem (Nexo Links, Nexo Agenda, Nexo ID, upcoming Nexo Events): short links on a dedicated short domain, cookieless click metrics, privacy by design, self-hostable. Alvaro's hosted instance: redirects on **nxo.li** (redirects ONLY — reputational fuse), panel/landing on **nexoshort.alvarocdev.com**. **Current state: Phases 1–3 implemented — core shortener + click metrics + the ADR-005 anti-abuse package (creation rate limits per user/IP, env-optional Safe Browsing, public `/report`, terms, operator moderation commands). 40 ACs (AC-1…AC-40) + Nexo ID SSO client integrated (16 AC tests, off by default); ADRs 001–008 Accepted, Gates 0–3 signed off (Alvaro, 2026-07-21); security review done. 106 tests, CI green. Next: Phase 4 (production deploy) + the redirect-URI re-registration blocker — owner/nexo-id, not autonomous.** Start at [docs/PLAN.md](docs/PLAN.md); architecture map in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
- **2026-07-22** — **SSO integration validated with nexo-id (coordination closed).** Redirect URI confirmed (panel-host callback; old nxo.li removed); `sub`/`email`/`email_verified`/`name` all ride in the **id_token** (no userinfo call — matches the template); login renders the `nexo_sso` error bag (Gate 3 dep, test-locked). nexo-id T4 done; back-channel logout is Phase-5-by-design. Pint docblock spacing fixed upstream (alvaro `09e7c7f`) → resolver re-synced byte-for-byte canonical. Nothing blocking on nexo-id. **Remaining is owner/infra only: Phase 4 deploy, add `/up` to UptimeRobot, flip repo public, and at launch set `NEXO_AUTH_MODE=sso` + `NEXO_SSO_ENABLED=true` (nexo-id runs its Gate 3 e2e from outside). Optional: local e2e vs nexo-id `:8100` before deploy, via Alvaro.** 117 tests, CI green.
- **2026-07-22** — **Phase 5 code slice done + open-source audit passed.** SSO-only auth mode (5.2, `EnsureLocalAuth`), SEO landing + brand assets + panel robots/sitemap (5.3, `PanelSeoController`; removed the static `public/robots.txt` that would override the short host Disallow in prod). 116 tests, CI green. `audit-open-source` **clean** (no secrets/keys/.env/sqlite in history or HEAD; public domains + OIDC client_id intentional). One low finding: commit author email `alvaro@mc4pc.com` would be public — owner's call (GitHub noreply for future commits; no history rewrite). Repo is publish-ready pending owner go + Phase 4 deploy.
- **2026-07-21** — **Nexo ID SSO client integrated (Phase 5.1).** nexo-id delivered the reusable `nexo-sso-client` template (`~/alvaro/templates/nexo-sso-client/`, its Phase 3). Copied **unmodified** (hard rule) into the app; adapted only allowed points (`intended`→`route('panel')`). Off by default (`NEXO_SSO_ENABLED=false`) → standalone intact. 16 AC tests (CFG/FLOW/LINK/SESS/DEGRADE) green; 106 tests total. Template pieces: `config/nexo-sso.php`, `routes/nexo-sso.php`, `app/Services/NexoSso/*`, `app/Http/Controllers/Auth/NexoSsoController.php`, `add_nexo_id_sub_to_users_table`. Template iterable-type docblocks fixed upstream (alvaro `3c86077`); resolver re-synced byte-for-byte and the temporary phpstan ignore removed. **Redirect URI corrected 2026-07-22** (client re-registered with the panel-host callback). **nexo-id T4 DONE 2026-07-22** → external launch condition met; nexoshort joins backups on deploy, only `/up` to add to UptimeRobot. Remaining Phase 5: SSO-only switch, SEO landing, `audit-open-source`, launch (needs Phase 4 deploy + owner).
